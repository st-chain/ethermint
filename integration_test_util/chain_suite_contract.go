...
var (
	...
	//go:embed compiled_contracts/2-wdym.json
	Contract2WDymJson []byte

	// Contract2WDym is the compiled WDYM contract
	Contract2WDym evmtypes.CompiledContract
...
func init() {
	...
	err = json.Unmarshal(Contract2WDymJson, &Contract2WDym)
	if err != nil {
		panic(err)
	}

	if len(Contract2WDym.Bin) == 0 {
		panic("load contract failed")
	}
...
func (suite *ChainIntegrationTestSuite) TxDeployErc20Contract(deployer *itutiltypes.TestAccount, name, symbol string, decimals uint8) (common.Address, *evmtypes.MsgEthereumTx, *itutiltypes.ResponseDeliverEthTx, error) {
	...
	return suite.TxDeployContract(
		...
		deployer,
		ERC20MinterBurnerDecimalsContract,
...
// TxDeploy2WDymContract deploys the embedded pre-compiled contract "2-wdym.sol".
func (suite *ChainIntegrationTestSuite) TxDeploy2WDymContract(deployer, rich *itutiltypes.TestAccount) (common.Address, *evmtypes.MsgEthereumTx, *itutiltypes.ResponseDeliverEthTx, error) {
	suite.Require().NotNil(deployer)
...
func (suite *ChainIntegrationTestSuite) TxDeploy5CreateBarInteractionContract(deployer *itutiltypes.TestAccount, contractBarAddress common.Address) (common.Address, *evmtypes.MsgEthereumTx, *itutiltypes.ResponseDeliverEthTx, error) {
	...
	_, _, err = suite.TxSendEvmTx(suite.CurrentContext, deployer, &addr, data)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
...
func (suite *ChainIntegrationTestSuite) TxDeployContract(ctx sdk.Context, deployer *itutiltypes.TestAccount, contract evmtypes.CompiledContract, constructorArgs ...interface{}) (common.Address, *evmtypes.MsgEthereumTx, *itutiltypes.ResponseDeliverEthTx, error) {
	if suite.TestConfig.DisabledContractCreation {
		panic("contract creation is disabled")
	}
	...
	evmTx, resDeliver, err := suite.TxSendEvmTx(ctx, deployer, nil, data)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
...
func (suite *ChainIntegrationTestSuite) TxMintErc20Token(contract common.Address, minter, mintTo *itutiltypes.TestAccount, amount uint16, decimals uint8) (*evmtypes.MsgEthereumTx, *itutiltypes.ResponseDeliverEthTx, error) {
	...
	data, err := ERC20MinterBurnerDecimalsContract.ABI.Pack("mint", mintTo.GetEthAddress(), mintAmt.BigInt())
	suite.Require().NoError(err)
	return suite.TxSendEvmTx(suite.CurrentContext, minter, &contract, data)
}
...
func (suite *ChainIntegrationTestSuite) TxTransferErc20Token(contract common.Address, sender, receiver *itutiltypes.TestAccount, amount uint16, decimals uint8) (*evmtypes.MsgEthereumTx, *itutiltypes.ResponseDeliverEthTx, error) {
	...
	return suite.TxSendEvmTx(suite.CurrentContext, sender, &contract, data)
}
...
func (suite *ChainIntegrationTestSuite) TxTransferErc20TokenAsync(contract common.Address, sender, receiver *itutiltypes.TestAccount, amount uint16, decimals uint8) (*evmtypes.MsgEthereumTx, error) {
	...
	data := suite.prepareTransferErc20TokenData(receiver, amount, decimals)
	return suite.TxSendEvmTxAsync(suite.CurrentContext, sender, &contract, data)
}
...
func (suite *ChainIntegrationTestSuite) prepareTransferErc20TokenData(receiver *itutiltypes.TestAccount, amount uint16, decimals uint8) []byte {
	...
	data, err := ERC20MinterBurnerDecimalsContract.ABI.Pack("transfer", receiver.GetEthAddress(), transferAmt.BigInt())
	suite.Require().NoError(err)
	return data
}
...
func (suite *ChainIntegrationTestSuite) TxSendEvmTx(ctx sdk.Context, sender *itutiltypes.TestAccount, to *common.Address, inputCallData []byte) (*evmtypes.MsgEthereumTx, *itutiltypes.ResponseDeliverEthTx, error) {
	msgEthereumTx := suite.prepareMsgEthereumTx(ctx, sender, to, nil, inputCallData)
...
func (suite *ChainIntegrationTestSuite) TxSendEvmTxAsync(ctx sdk.Context, sender *itutiltypes.TestAccount, to *common.Address, inputCallData []byte) (*evmtypes.MsgEthereumTx, error) {
	msgEthereumTx := suite.prepareMsgEthereumTx(ctx, sender, to, nil, inputCallData)
...
func (suite *ChainIntegrationTestSuite) prepareMsgEthereumTx(ctx sdk.Context, sender *itutiltypes.TestAccount, to *common.Address, amount *big.Int, inputCallData []byte) *evmtypes.MsgEthereumTx {
	suite.Require().NotNil(sender)
	...
	msgEthereumTx := evmtypes.NewTx(
		suite.ChainApp.EvmKeeper().ChainID(),
		suite.ChainApp.EvmKeeper().GetNonce(ctx, from),
		to,
		amount,
		gas,
		nil,
		suite.ChainApp.FeeMarketKeeper().GetBaseFee(ctx),
		big.NewInt(1),
		inputCallData,
		&ethtypes.AccessList{},
	)
